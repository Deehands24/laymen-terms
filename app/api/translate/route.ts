...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      // Execute submission and translation in parallel to reduce latency
      // This saves the duration of the DB write (~50-100ms) from the total request time
      ...
      // Calculate remaining count locally to avoid an extra DB call
      const remaining = limit === -1 ? -1 : Math.max(0, initialRemaining - 1)
