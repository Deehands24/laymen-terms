...
export function TranslationForm({ userId }: TranslationFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If this was a partial success (fallback mode), show a warning
  ...
  return (
    <div className="space-y-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {error && (
          <div
            className={`border-l-4 p-4 ${error.includes("Note:") ? "bg-yellow-50 border-yellow-400" : "bg-red-50 border-red-400"}`}
          >
            <div className="flex">
              <div className="flex-shrink-0">
                <svg
                  className={`h-5 w-5 ${error.includes("Note:") ? "text-yellow-400" : "text-red-400"}`}
                  viewBox="0 0 20 20"
                  fill="currentColor"
                >
                  {error.includes("Note:") ? (
              ...
              <div className="ml-3">
                <p className={`text-sm ${error.includes("Note:") ? "text-yellow-700" : "text-red-700"}`}>{error}</p>
